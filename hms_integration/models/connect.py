...
class connect(models.Model):
    ...
    # Connect Function that call from connect-xml
    def connect_function(self):
        self.env['sales.order.lines.integration'].sudo().with_context(active_test=False).search([]).unlink()
        self.env['sales.order.integration'].sudo().with_context(active_test=False).search([]).unlink()
